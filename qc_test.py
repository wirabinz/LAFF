...
def test_rcopy(x, y):
    print("\ncolumn -> row copy")
    if np.array_equal(laff.copy(x, y[:, np.newaxis]), x[:, np.newaxis]):
        print("TEST PASSED")
    else:
        print("TEST FAILED")
...
def test_c2rcopy(x, y):
    print("\nrow -> column copy")
    if np.array_equal(laff.copy(x[:, np.newaxis], y), x):
        print("TEST PASSED")
    else:
        print("TEST FAILED")
...
def test_r2rcopy(x, y):
    print("\nrow -> row copy")
    if np.array_equal(laff.copy(x[:, np.newaxis], y[:, np.newaxis]), x[:, np.newaxis]):
        print("TEST PASSED")
    else:
        print("TEST FAILED")
...
print(laff.copy(x, z[:,np.newaxis]))

print("\nrow -> column copy (wrong size)")
print(laff.copy(x[:,np.newaxis], z))

print("\nrow -> row copy (wrong size)")
print(laff.copy(x[:,np.newaxis], z[:,np.newaxis]))
